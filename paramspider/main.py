...
# def sessionFile(url , results_dir):
#     session_file = f"Session : {timestamp}.txt" # session file name
#     # if not os.path.exists(f"{results_dir}/{session_file}"):
#     s = open(f"{results_dir}/{session_file}" , "w")
#     s.write(url)
#     # else :
#     #     s.write(url)
...
def fetch_and_clean_urls(domain, extensions, stream_output,proxy, placeholder):
    ...
    # session_file = os.path.join(results_dir, f"Session : {timestamp}.txt")
    # def sessionFile(url):
    # session_file = f"Session : {timestamp}.txt" # session file name
    # if not os.path.exists(f"{results_dir}/{session_file}"):
    #     s = open(f"{results_dir}/{session_file}" , "w")
